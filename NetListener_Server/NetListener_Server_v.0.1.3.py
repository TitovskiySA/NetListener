        ...
        #               ServerPort]
            ...
            for String in Strings:
                ...
                if String[0] == "MyPos":
                    ...
                    temp = (String[1].split()[0], String[1].split()[1])[:]
                    if self.Settings[2] == [("192.168.1.1", "12345678")]:
                        self.Settings[2] = [temp]
                    else:
                        self.Settings[2].append(temp)
                elif String[0] == "WAVformat":
                    self.Settings[3] = [int(String[1].split()[0]), self.StrCodeToAudio(String[1].split()[1]), int(String[1].split()[2]), int(String[1].split()[3]), int(String[1].split()[4])]
    ...
    def OpenSettings(self, evt):
        try:
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                self.CreateThreads(QueueThread = False)
...
#=======================================================================================
    ...
    def OnCloseWindow(self, evt):
        ...
        try:
            ...
            self.OffThread(self.RecThread)
            self.ConnThread.OffConnThread()
            self.RecThread.join()
            self.ConnThread.join()
            self.StopStream()
            self.QueueThread.OffThread()
            ...
            try:
                ...
                ToLog("Error in OffThreadfunctions, Error code = " + str(Err))
            wx.Exit()
            sys.exit()
            #raise Exception
#=======================================================================================
...
#=========================================
    def CreateThreads(self, QueueThread = True):
        try:
            ...
            if QueueThread == True:
                self.QueueThread = MultiQThread(self, self.stream, self.Settings[3][0])
                threads = [self.RecThread, self.ConnThread, self.QueueThread]
            else:
                threads = [self.RecThread, self.ConnThread]
...
class ConnThread(MyThread):
    ...
    def TCPServer(self):
        ...
        try:
            ...
            try:
                ...
                ToLog("Error in StartNewThread, Error code = " + str(Err))
                raise Exception
    ...
    def ResumeConn(self):
        try:
            ...
            ToLog("Error in ResumeConn, Error code = " + str(Err))
            raise Exception
    ...
    def PauseConn(self):
        try:
            ...
            ToLog("Error in PauseConn, Error code = " + str(Err))
            raise Exception

    #---------------------------------------
    def OffConnThread(self):
        try:
            ToLog("I'm " + self.name + " and I have stop command")
            if len(self.threads) > 0:
                for thread in self.threads:
            ...
            ToLog("Error in OffConnThread, Error code = " + str(Err))
            raise Exception

    #----------------------------------------
    def StartNewThread(self):
        ToLog("StartNewThread with number = " + str(self.numOfThread))
        Thread = SendFrameThread(self, self.stream, self.settings, self.sock, [self.conn, self.addr], self.socktimeout, self.numOfThread)
        Thread.setDaemon(True)
        self.threads.append([self.numOfThread, Thread])
        self.connlist.append([self.numOfThread, self.conn, self.addr])
        Thread.start()
        self.numOfThread = self.numOfThread + 1
        
        
        text = "Подключился новый клиент с данными: " + str(self.conn)
        text = text + "\n Полный перечень подключенных клиентов:"
        for connect in self.connlist:
            text = text + "\n\t" + str(connect)
        
        self.ToWin(["showWin", text])

    #----------------------------------------
    def MsgOfChild(self, mess):
        try:
            if isinstance(mess, list):
                if mess[1] == "finished":
                    self.KillThread(mess[0])

        except Exception as Err:
            ToLog("Error in MsgOfChild, Error code = " + str(Err))
            raise Exception

    #----------------------------------------
    def KillThread(self, num):
        temp = []
        for thread in self.threads:
            if num == thread[0]:
                continue
        ...
        text = "Отключился клиент с данными: " + str(deleted)
        text = text + "\n Полный перечень подключенных клиентов:"
        for connect in self.connlist:
            text = text + "\n\t" + str(connect)
...
class SendFrameThread(threading.Thread):
    def __init__(self, parent, stream, settings, sock, connection, timeout, numOfThread):
        super().__init__()
        self.parent = parent
        self.stream = stream
        self.settings = settings
        ...
        self.socktimeout = timeout
        self.num = numOfThread
        self.stop = False
        self.name = "SendFrameThread#" + str(self.num)
        
    def run(self):
        ToLog(self.name + " started")
        global MQueue
        MQueue.AddQueue(self.num)
        while True:
            if self.stop == True:
                break
            #print("===============iter SendFrameThread==================")
            try:
                self.TransvTCP()
            except socket.error as Err:
                ToLog("Socket Error in self.TransvTCP, Error code = " + str(Err))
                break
                
            except Exception as Err:
                ToLog("Error in self.TransvTCP, Error code = " + str(Err))
                raise Exception
                break
        ...
        try:
            ...
            ToLog("Error in closing connection in " + self.name)
            raise Exception

        try:
            self.parent.MsgOfChild([self.num, "finished"])
        except Exception as Err:
            ToLog("Error in MsgOfChild from " + self.name)
            
        ToLog(self.name + " finished")

    #---------------------------------------
    def TransvTCP(self):
        global MQueue
        ToLog("TransvTCP function started in " + self.name)
        data = None
        while True:
            if self.stop == True:
                break

            #ToLog("Getting data from MQueue num#" + str(self.num))
            #ToLog("MQueueNumbers are = " + str(MQueue.AliveNum))
            data = MQueue.Get(self.num)
            a = pickle.dumps(data)
            message = struct.pack("Q", len(a)) + a
            self.conn.sendall(message)
        ToLog("TransvTCP function finished in " + self.name)

    #---------------------------------------
    def StopThread(self):
        try:
            global MQueue
            ToLog("StopThread started in " + self.name)
            MQueue.RemoveQueue(self.num)
            self.stop = True

        except Exception as Err:
            ToLog("Error in StopThread in " + self.name + ", Error code = " + str(Err))
...
#======================
#SendFrameThread
class MultiQThread(threading.Thread):
    def __init__(self, parent, stream, chunk, name = "MQueueThread"):
        super().__init__()
        self.stop = False
        self.parent = parent
        self.stream = stream
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                ToLog("Error in ReadDaraFromStream, Error code = " + str(Err))
                raise Exception

        ToLog(self.name + " finished")
        
    #---------------------------------------
    def ReadDataFromStream(self):
        global MQueue
        #data = self.stream.read(self.chunk)
        MQueue.Put(self.stream.read(self.chunk))
                
    #---------------------------------------
    def RefreshPams(self, stream, chunk):
        try:
            ToLog("RefreshPams func in " + self.name + " started")
            self.stream = stream
            self.chunk = chunk
            ToLog("RefreshPams func in " + self.name + " finished")
        except Exception as Err:
            ToLog("Error in RefreshPams in " + self.name + "Error code = " + str(Err))
            raise Exception
        
    #---------------------------------------
    def StopThread(self):
        try:
            ToLog("StopThread function started in " + self.name)
            self.stop = True
...
class Multiqueue(list):
    def __init__(self, AliveNum = [0], Queues = [queue.Queue()]):
        list.__init__(self)
        print("New MultiQueue object created")
        self.AliveNum = AliveNum
        for num in self.AliveNum:
            self.append(queue.Queue())
        print("Mylen = " + str(len(self)))

    #---------------------------------------
    def AddQueue(self, num):
        try:
            if num in self.AliveNum:
                ToLog("Queue#" + str(num) + "is already in MultiQueue")
                return False
            else:
                self.append(queue.Queue())
                self.AliveNum.append(num)
                ToLog("Adding Queue#" + str(num) + " successed, now Queue numbers are = " + str(self.AliveNum))
                return True
        except Exception as Err:
            ToLog("Error in AddQueue, Error code = " + str(Err))
            return False
            #raise Exception
        
    #---------------------------------------
    def RemoveQueue(self, num):
        try:
            if num not in self.AliveNum:
                ToLog("MultiQueue doen't consist queue with num = " + str(num))
                return False
            elif num == 0:
                ToLog("MultiQueue must not delete queue with num = " + str(num))
                return False
            else:
                tempNum = []
                tempQueue = self[:]
                self.clear()
                for i in range (0, len(self.AliveNum)):
                    if self.AliveNum[i] == num:
                        continue
                    else:
                        tempNum.append(self.AliveNum[i])
                        self.append(tempQueue[i])
                self.AliveNum = tempNum[:]
                #print(self.AliveNum)
                
                ToLog("Removing Queue#" + str(num) + " successed, now Queue numbers are = " + str(self.AliveNum))
                return True
                
        except Exception as Err:
            ToLog("Error in Remove Queue, Error code = " + str(Err))
            return False
            #raise Exception

    #---------------------------------------
    def RefreshPams(self, listnums = [-1]):
        try:
            if listnums == [-1]:
                self.AliveNum = [0]
            elif 0 in listnums:
                self.AliveNum = listnums
            else:
                self.AliveNum = [0] + listnums
            self.clear()
            for i in range(0, len(listnums) + 1):
                self.append(queue.Queue())
            print("Pams of MultiQueue Refreshed, now Queue numbers are = " + str(self.AliveNum))
            return True

        except Exception as Err:
            print("Error in RefreshPams, Error code = " + str(Err))
            return False
            #raise Exception

    #---------------------------------------
    def Put(self, elm, num = -1):
        try:
            if len(self) > 0:
                if num == -1:
                    for queues in self:
                        queues.put(elm)
                    #ToLog("Successed elm to all Queues of MultiQueue")
                    return True
                else:
                    if num not in self.AliveNum:
                        ToLog("MultiQueue doen't consist queue with num = " + str(num))
                        ...
                        self[num].put(elm)
                        #ToLog("Successed elm to Queue#" + str(num) + " of MultiQueue")
                        return True
                              
            else:
                ToLog("When putting item found num of queues = 0")
                return False
                
        except Exception as Err:
            ToLog("Error in Put MultiQueue, Error code = " + str(Err))
            return False
    ...
    def Get(self, num):
        try:
            if num not in self.AliveNum:
                ...
                for i in range (0, len(self.AliveNum)):
                    if num == self.AliveNum[i]:
                        return self[i].get()
        except Exception as Err:
            ToLog("Error in Get MultiQueue, Error code = " + str(Err))
            #raise Exception
            return False

    #---------------------------------------
    def CallQueue(self, num):
        try:
            if num not in self.AliveNum:
                ToLog("MultiQueue doen't consist queue with num = " + str(num))
                return False
            ...
            ToLog("Error in Get CallQueue, Error code = " + str(Err))
            #raise Exception
            return False
...
class ChangeClickThrob(wx.lib.throbber.Throbber):
    def __init__(self, parent, ListOfFrames, NumOfFrames, FrameDelay, AnswerTime = 200, AnswerFrame = False):
        wx.lib.throbber.Throbber.__init__(self, parent, wx.ID_ANY, ListOfFrames, frames = NumOfFrames, frameDelay = FrameDelay)
        print("New ChangeClickThrob object created")
        self.CurFrame = 0
        self.AnswerTime = AnswerTime
        if AnswerFrame == False:
            self.AnswerFrame = NumOfFrames - 1
...
global LogDir, MyDir, MyDate,LogQueue, MQueue
