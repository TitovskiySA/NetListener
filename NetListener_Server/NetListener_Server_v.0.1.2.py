...
class ConnThread(MyThread):
    ...
    def StartNewThread(self):
        ...
        self.aliveNum.append(self.numOfThread)
        Thread.start()
...
class MultiQThread(threading.Thread):
    def __init__(self, parent, stream, chunk, name = "MultiThread", numQueue = 0):
        super().__init__()
        self.stop = False
        self.parent = parent
    ...
    def SetNum(self, num):
        try:
            ToLog("SetNum function in " + self.name + " started")
            global MultiQueue
            MultiQueue.clear()
            for i in range(0, num):
                MultiQueue.append(queue.Queue())
            #self.numQueue = num

            ToLog("SetNum function in MultiQThread finished, num = " + str(num))
        except Exception as Err:
            ToLog("Error in SetNum in " + self.name + "Error code = " + str(Err))
            raise Exception
