...
import struct #for coding and  trnsiving bytes
...
class MainWindow(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        super().__init__(
            parent,
            title = "NetListener Client ver 0.0.1",
            #size = self.SizeWin,
            style = styleWindow)
        #------------------------------------------------------------------------------
        #задаем иконку
        frameIcon = wx.Icon(os.getcwd() + "\\images\\banner_client.png")
...
class MainPanel(wx.Panel):
    ...
    def CreatePanel(self):
        ...
        self.Throb.SetToolTip("Сервер не подключен")
        ...
        self.ConnectBtn = ChangeClickThrob(
            self, [BMPs[0], BMPs[1], BMPs[2]], 3, 1, AnswerTime = 100)
        ...
        SetBtn.SetToolTip("Окно настроек аудио")
        SetBtn.Bind(wx.EVT_BUTTON, self.OpenSoundSettings)
        SetBtn.SetMinSize(ButtonSize)

        #NetworkSettingsBtn
        NetSetBtn = wx.BitmapButton(self, wx.ID_ANY, BMPs[4])
        NetSetBtn.SetToolTip("Окно сетевых настроек")
        NetSetBtn.Bind(wx.EVT_BUTTON, self.OpenNetSettings)
        NetSetBtn.SetMinSize(ButtonSize)
        ...
        CommonVbox.Add(NetSetBtn, 0, wx.ALL, 3)
...
#=====================================================================================
    ...
    def Preparing(self):
        ...
        #starting thread for saving frames
        self.SavingThread = SaveFrameThread(self.p, self.stream, self.Settings)
...
#==================================================================================
    def DoLoad(self, evt):
        ToLog("Manual Load function started")
        self.LoadConfig()
#==================================================================================
        ...
        try:
            ...
            for String in Strings:
                ...
                if String[0] == "MyPos":
                    ...
                    self.Settings[2] = (String[1].split()[0], int(String[1].split()[1]))[:]
                elif String[0] == "WAVformat":
                    self.Settings[3] = [
                        int(String[1].split()[0]), self.StrCodeToAudio(String[1].split()[1]),
                        int(String[1].split()[2]), int(String[1].split()[3]), int(String[1].split()[4])]
                    
            ToLog("Succesfully loaded Settings:")
            for strings in self.Settings:
                ToLog("\t" + str(strings), NoDate = True)
...
#===============================================================================
    def DoSave(self, evt):
        ToLog("Manual Save function started")
        self.SaveConfig()
#===============================================================================
    #save config
    def SaveConfig(self, Auto = False):
        ...
        try:
            ...
            file.write("Server " + str(self.Settings[2][0]) + " " +
                       str(self.Settings[2][1]) + "\n")
            ...
            wx.MessageBox("Ошибка при сохранении профиля, код ошибки = " + str(Err), " ", wx.OK)
            raise Exception
        return
#=======================================================================================
    #Licence
...
#==============================================================================================
    def OpenInfo(self, event):
        ToLog("Open Info button pressed")
        try:
            ...
            try:
                ...
                ToLog("Error in OpenInfo function, Error code = " + str(Err))
        return
    
#===========================================================================================
    #opening settings
    def OpenSoundSettings(self, evt):
                            #self.Settings = [
            #               [MainPos],
            #               folder for records,
            #               (ipServer, portServer),
...
#========================================================================================
    #opening net settings
    def OpenNetSettings(self, evt):
        print("write new WIn for it")
        wx.MessageBox("write someth here", " ", wx.OK)
...
#=======================================================================================
    def RClickThrob(self, evt):
        ...
        try:
            ...
            self.ToLog("Error of Popup menu because of = " + str(Err))
        return
#=========================================
    #creating stream
...
#=========================================
    def CreateThreads(self):
        try:
            ...
            if self.SavingThread.is_alive() == False:
                self.SavingThread = SaveFrameThread(self.p, self.stream, self.Settings)
                self.SavingThread.setDaemon(True)
                self.SavingThread.start()
                
            ToLog("CreateThreads function completed")
...
#===================================
    ...
    def ClickedConn(self, evt):
        try:
            if self.TryThread(self.ConnThread) == "dead":
                ToLog("Connect Button Pressed, thread was dead, ConnThread is switched to On")
                self.CreateThreads()
                self.evtConn.set()
                self.ConnectBtn.Clicked(1)
                self.ConnectBtn.SetToolTip("Connect is On")
            
            elif self.TryThread(self.ConnThread) == "waiting":
                ToLog("Connect Button Pressed, thread was paused, ConnThread is switched to On")
                ...
                ToLog("Connect Button Pressed, ConnectThread and Record is switched to Off")
...
#=============================================================================================
    ...
    def UpdateDisplay(self, mess):
        try:
            ...
            if mess == "EndOfConnThread":
                self.ConnectBtn.Clicked(0)
                self.ConnectBtn.SetToolTip("Connect is Off")
                wx.MessageBox("Disconnected from NetListener server", " ", wx.OK)

            else:
                ToLog("Unexpected message in UpdateDisplay function = " + str(mess))
...
class MyThread(threading.Thread):
    def __init__(self, myname, evtCon, pams):
        super().__init__()
        self.pams = pams
        self.myname = myname
        self.stop = False
    ...
    #------------------------------------------------------
    def OffThread(self):
        ToLog("I'm " + self.myname + " and I have stop command")
        self.stop = True
        self.evtCon.set()

    #------------------------------------------------------
    def Pause(self):
        ToLog("I'm " + self.myname + " and I have pause command")
        self.evtCon.clear()
        

    #-------------------------------------------------------
    def Resume(self):
        ToLog("I'm " + self.myname + " and I have resume command")
        self.evtCon.set()

    #-------------------------------------------------------
    def RefreshPams(self, pams):
        ToLog("I'm " + self.myname + " and I have refresh pams command")
        self.pams = pams
        for Pam in self.pams:
            ToLog(str(Pam), NoDate = True)

    #-------------------------------------------------------
    def TryThread(self):
        if self.stop == True:
            return "dead"
        elif self.evtCon.isSet():
            return "running"
        else:
            return "waiting"
    ...
    def SendMess(self, mess):
        try:
            wx.CallAfter(pub.sendMessage, "UpdateMainWin", mess = mess)
        except Exception as Err:
            ToLog("Error in SendMess function, Error  code = " + str(Err))
...
class ConnThread(MyThread):
            ...
            #               (ipServer, portServer),
            #               [CHUNK, FORMAT, CHANNELS, RATE, seconds]]
    def __init__(self, name, evtCon, p, stream, settings, tcp = True, timeout = 3, numclient = 5):
        super().__init__(name, evtCon, settings)
    ...
    def run(self):
        ...
        try:
            ...
            ToLog("Error in self.StartClient, Error code = " + str(Err))
            #raise Exception

        try:
            self.sock.close()
        except Exception as Err:
            ToLog("Error closing socket, Error code = " + str(Err))
        ToLog("Connect Thread finished")
        self.SendMess("EndOfConnThread")
        
    #--------------------------------------
    def StartClient(self):
        ToLog("StartClient function started")
        self.evtCon.wait()
        if self.stop == True:
            return
        while True:
            self.evtCon.wait()
            if self.stop == True:
                ToLog("StartClient function finished by stop command")
                break
            if not self.evtCon.isSet():
                ToLog("ConnThreasd giong to sleep by event command")
                continue
            if self.tcp == True:
                ToLog("Started TCP Client with Hostname = " + str(socket.gethostname()))
                self.TCPClient()
            else:
                ToLog("Started UDP Client with Hostname = " + str(socket.gethostname()))
                self.UDPClient()
        ToLog("StartClient function finished")
            
    #---------------------------------------
    def TCPClient(self):
        ToLog("TCPClient function started")
        #create socket with lifetime self.socktimeout sec
        ...
        try:
            ...
            ToLog("Error creating TCP socket, Error code = " + str(Err))
            #raise Exception
        ...
        try:
            ...
            ToLog("Error in ReceiveTCP function, Error code = " + str(Err))
            #raise Exception
        self.sock.close()
        ToLog("TCPClient function finished")
       
    #---------------------------------------
    def ReceiveTCP(self):
        ToLog("ReceiveTCP function started")
        self.data = b""
        self.payload_size = struct.calcsize("Q")
        while True:
            if self.stop == True or not self.evtCon.isSet():
                ToLog("ReceiveTCP function finished by stop event command")
                break
            try:
                self.ReceiveDataFromTCP()
            except socket.error as Err:
                ToLog("Socket Error in ReceiveDataFromTCP, Error code = " + str(Err))
                break
            except Exception as Err:
                ToLog("Error in ReceiveDataFromTCP, Error code = " + str(Err))
                break
                #raise Exception
        ToLog("ReceiveTCP function finished")

    #----------------------------------------
    def ReceiveDataFromTCP(self):
        #ToLog("ReceiveDaraFromTCP function started")
        while len(self.data) < self.payload_size:
            try:
                packet = self.sock.recv(4*1024)
            except socket.timeout:
                ToLog("ReceiveDaraFromTCP function finished by timeout")
                self.stop = True
                return
            if not packet or not self.evtCon.isSet():
                ToLog("Stop command in ReceiveDataFromTCP")
                self.stop = True
                break
            self.data += packet

        packed_mess_size = self.data[:self.payload_size]
        self.data = self.data[self.payload_size:]
        msg_size = struct.unpack("Q", packed_mess_size)[0]
        ...
        while len(self.data) < msg_size:
            if self.stop == True or not self.evtCon.isSet():
                ...
                #break
                return
            self.data += self.sock.recv(4*1024)
        ...
        self.WriteFrames(frame)
        #ToLog("ReceiveDaraFromTCP function finished")

    #----------------------------------------
    def UDPClient(self):
        ToLog("UDPClient function started")
        #create socket with lifetime 1 sec
        self.UDPqueue = queue.Queue(maxsize = 2000)
        ...
        try:
            #mess = b'Hello'
            mess = str.encode("Hello")
            self.q = queue.Queue(maxsize=2000)
            sock_addr = self.settings[2]
            print("ADRRR = " + str(sock_addr))
            self.sock.sendto(mess, sock_addr)
        except Exception as Err:
            ToLog("Error creating UDP socket, Error code = " + str(Err))
            #raise Exception

        try:
            self.ReceiveUDP()
        except Exception as Err:
            ToLog("Error in Receive UDP function, Error code = " + str(Err))
            #raise Exception
        self.sock.close()
        ToLog("UDPClient function finished")

    #----------------------------------------
    def ReceiveUDP(self):
        ToLog("Receive UDP func started")
        thr = threading.Thread(target = self.getAudioUDP, args = ())
        thr.setDaemon(True)
        thr.start()
        ...
        while True:
            if self.stop == True or not self.evtCon.isSet():
                ToLog("End of ReceiveUDP by command")
                break
            frame = self.q.get()
            ToLog("Putting frame to stream")
            self.stream.write(frame)
            self.WriteFrames(frame)
            #send frame to RecThread
        ToLog("ReceiveUDP function finished")

    #----------------------------------------
    def getAudioUDP(self):
        ToLog("GetAudioUDP thread started")
        while True:
            if self.stop == True or not self.evtCon.isSet():
                ToLog("End of getAudioUDP thread by command")
                break
            frame, _ = self.sock.recvfrom(self.BUFF_SIZE)
            self.q.put(frame)
            ToLog("frame to queue putted")
        ToLog("GetAudioUDP function finished")
    ...
    def WriteFrames(self, frame):
        try:
            ...
            ToLog("Error in WriteFrames, Error code = " + str(Err))
            #raise Exception
...
#=============================================
# Tolog - renew log
def ToLog(message, NoDate = False):
    try:
        global LogQueue
        if NoDate == False:
            LogQueue.put(str(datetime.today())[10:19] + "  " + str(message) + "\n")
        else:
            LogQueue.put("\t" + str(message) + "\n")
    except Exception as Err:
...
#=============================================
# Frame to file Queue
def FrameToFile(frame):
    try:
        global FrameQueue
        FrameQueue.put(frame)
    except Exception as Err:
        ToLog("Error in FrameToFile function, Error code = " + str(Err))
        raise Exception
...
#=============================================
# Thread for saving logs
class SaveFrameThread(threading.Thread):
    def __init__(self, p, stream, settings):
        super().__init__()
        self.framesToWrite = []
        self.p = p
        self.stream = stream
    ...
    def run(self):
        #global FrameQueue
        ToLog("SaveFrameThread started!!!")
        self.writingQueue()
        ToLog("SaveFrameThread finished!!!")
            ...
            try:
                if FrameQueue.empty():
                    ...
                    while not FrameQueue.empty():
                        self.framesToWrite.append(FrameQueue.get_nowait())
                        cycle = cycle + 1
                        #print("curcycle = " + str(cycle) + " < " + str(self.settings[3][3] / self.settings[3][0] * self.settings[3][4]))
                    
                    if cycle >= int(self.settings[3][3] / self.settings[3][0] * self.settings[3][4]):
                        #print("CYCLE = " + str(cycle))
                        try:
                            self.writeToFile(self.framesToWrite)
                        except Exception as Err:
                            ToLog("Error in writeToFile function, Error code = " + str(Err))
                        self.framesToWrite = []
                        cycle = 0
                        
            except Exception as Err:
                ToLog("Error in writingQueue, Error code = " + str(Err))
                raise Exception
    #------------------------------------------
    def RefreshPams(self, p, pams):
        try:
            self.settings = pams
            self.p = p
            ToLog("Refreshing Pams in SaveFrameThread, new Pams:")
            for Pams in pams:
                ToLog("\t" + str(Pams), NoDate = True)
            
        except Exception as Err:
            ToLog("Error in RefreshPams in SaveFrameThread, Error code = " + str(Err))
            raise Exception

    #------------------------------------------
    def writeToFile(self, frames):
        ToLog("writeToFile function Started")
        ...
        ToLog("writeToFile function Finished")
...
global LogDir, MyDir, MyDate, LogQueue, FrameQueue
